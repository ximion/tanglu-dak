...
t <- (read.table("/srv/dak/misc/ftpstats.data",sep=",",header=0,row.names=1,col.names=cname))/1024/1024
v <- t[(length(t$all)-90):(length(t$all)),1:19]

plotsize("/srv/ftp-master.tanglu.org/web/size-quarter.png","Daily dinstall run size by arch (past quarter)",v)

...
class DEP11YAMLDumper(yaml.Dumper):
    '''
    Custom YAML dumper, to ensure resulting YAML file can be read by all parsers
    (even the Java one)
    '''
...
class ComponentData:
    ...
    def add_provided_item(self, kind, value):
        if not kind in self.provides.keys():
                ...
                try:
                    ...
                    if key.startswith('Keywords'):
                        ...
                        if key[8:] == '':
                            if compdata.keywords:
                                if set(value) not in [set(val) for val in compdata.keywords.values()]:
                                    compdata.keywords.update({'C':map(enc_dec,value)})
                            else:
                                compdata.keywords = {'C':map(enc_dec,value)}
                        else:
                            if compdata.keywords:
                                if set(value) not in [set(val) for val in compdata.keywords.values()]:
                                    compdata.keywords.update({key[9:-1]:map(enc_dec,value)})
                            else:
                                compdata.keywords = {key[9:-1]:map(enc_dec,value)}
                        continue
                    ...
                    if key == 'MimeType':
                        ...
                        if len(value) > 1:
                            value.pop()
                        for val in value:
                            compdata.add_provided_item(ProvidedItemType.MIMETYPE, val)
                        continue
        ...
        for usubs in subs:
            ...
            if usubs.tag == 'ul' or usubs.tag == 'ol':
                ...
                for u_usubs in usubs:
                    ...
                    if u_usubs.tag == 'li':
                        if dic.get(key):
                            dic[key] += "<li>%s</li>" % self.neat(enc_dec(u_usubs.text))
                        else:
                            dic[key] =  "<%s><li>%s</li>" % (usubs.tag,self.neat(enc_dec(u_usubs.text)))
    ...
    def read_xml(self,xml_content,compdata):
        ...
        for subs in root:
            ...
            if subs.tag == "provides":
                for bins in subs:
                    if bins.tag == "binary":
                        compdata.add_provided_item(ProvidedItemType.BINARY, bins.text)
                    if bins.tag == 'library':
                        compdata.add_provided_item(ProvidedItemType.LIBRARY, bins.text)
                    if bins.tag == 'dbus':
                        compdata.add_provided_item(ProvidedItemType.DBUS, bins.text)
                    if bins.tag == 'firmware':
                        compdata.add_provided_item(ProvidedItemType.FIRMWARE, bins.text)
                    if bins.tag == 'python2':
                        compdata.add_provided_item(ProvidedItemType.PYTHON_2, bins.text)
                    if bins.tag == 'python3':
                        compdata.add_provided_item(ProvidedItemType.PYTHON_3, bins.text)
                    if bins.tag == 'codec':
                        compdata.add_provided_item(ProvidedItemType.CODEC, bins.text)
    ...
    def read_metadata(self,suitename,component,binid,filelist,pkg):
        ...
        if self._loxml:
            for meta_file in self._loxml:
                compdata = ComponentData(suitename,component,binid,self._filename,filelist,pkg)
...
class ContentGenerator:
    ...
    def write_meta(self,dic,ofile,dep11):
        ...
        metadata = yaml.dump(dic, Dumper=DEP11YAMLDumper,
                             default_flow_style=False, explicit_start=True,explicit_end=False,
                             width=100, indent=2,allow_unicode=True)
    ...
    def scale_screenshots(self,imgsrc,path):
        '''
        scale images in three sets of two-dimensions(752x423 624x351 and 112x63)
        '''
        thumbnails = []
        ...
        for size in sizes:
            ...
            thumbnails.append({'url':url,'height':int(ht),'width':int(wd)})
        ...
        if self._cdata.screenshots:
            ...
            for shot in self._cdata.screenshots:
                try:
                    ...
                    path = "%s%s-%s/screenshots/" % ( path,self._cdata._pkg, str(self._cdata._binid))
                    ...
                    shot['source-image']['url'] = self.make_url('%ssource/screenshot-%s.png' % (path,hd))
                    img.close()
                    #scale_screenshots will return a list of dicts with {height,width,url}
                    shot['thumbnails']=self.scale_screenshots('%ssource/screenshot-%s.png' % (path,hd),path)
                    shots.append(shot)
                    success.append(True)
    ...
    def save_icon(self,icon,filepath,values):
        ...
        template = Config()["Dir::MetaInfo"]+"%(suite)s/%(component)s/"
        path = template % values
        path = "%s%s-%s/icons/" %( path,self._cdata._pkg,str(self._cdata._binid))
                ...
                for path in self._cdata._filelist:
                    if path.endswith(ext_allowed):
                        if 'pixmaps' in path or 'icons' in path:
                            return self.save_icon(path,self._cdata._file,values)
...
def make_icon_tar(suitename,component):
    ...
    '''
    location = "%s%s/%s/*/icons/" % (Config()["Dir::MetaInfo"],suitename,component)
    tar_location = "%sdists/%s/%s/" % (Config()["Dir::Root"],suitename,component)
    ...
    for arch in datalist.arch_deblist.iterkeys():
        ...
        for key in datalist.arch_deblist[arch]:
            ...
            if os.path.exists(path+key):
                ...
                cd_list = mde.read_metadata(suitename, component, str(info_dic[key]), filelist, pkg_list[key])
